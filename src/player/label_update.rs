...
use crate::universe::{Star, Planet};
...
fn update_billboards(
    mut q_billboards: Query<(&GlobalTransform, &mut Transform), (With<Billboard>, With<SystemLabel>)>, 
    q_camera: Query<&GlobalTransform, With<crate::player::camera::ZenCamera>>,
) {
    let Ok(cam_tf) = q_camera.get_single() else { return; };
    let cam_pos = cam_tf.translation();

    for (global_tf, mut local_tf) in q_billboards.iter_mut() {
...
fn update_system_labels(
    ...
    mut q_labels: Query<(Entity, &mut Text2d, &Parent, Option<&mut LastLabelUpdate>), With<SystemLabel>>,
    ...
    for (entity, mut text, parent, last_update) in q_labels.iter_mut() {
        ...
        if let Some(mut last) = last_update {
            if now - last.0 < 1.0 { continue; }
            last.0 = now;
        } else {
            commands.entity(entity).insert((LastLabelUpdate(now), Billboard)); // Add Billboard here!
        }

        if let Ok(object_parent) = q_hierarchy.get(**parent) {
             // Determine Type
             let (is_star, is_planet) = if let Ok((s, p)) = q_parents.get(**parent) {
                 (s.is_some(), p.is_some())
             } else { (false, false) };
                         ...
                         Ok(Some(discovery)) => {
                             ...
                             if discovery.name == default_name {
                                 ...
                                 if is_planet {
                                     ...
                                     if text.0 != planet_name { text.0 = planet_name; }
                                 } else {
                                     // Star keeps "S ..."
                                     if text.0 != default_name { text.0 = default_name; }
                                 }
                             } else {
                                 // Custom Name "MyHome"
                                 ...
                                 if text.0 != discovery.name {
                                     if is_planet {
                                          ...
                                          if text.0 != p_name { text.0 = p_name; }
                                     } else {
