...
impl Plugin for SkyPlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<StarfieldConfig>()
            ...
            .add_systems(
                Update,
                (spawn_sky_sphere, update_sky_position, sync_starfield_config),
            );
    }
}
...
fn update_sky_position(
    ...
    let Ok((cell, transform)) = q_camera.get_single() else {
        return;
    };
    let Ok(sky_mat_handle) = q_sky.get_single() else {
        return;
    };
    let Some(material) = materials.get_mut(sky_mat_handle) else {
        return;
    };
